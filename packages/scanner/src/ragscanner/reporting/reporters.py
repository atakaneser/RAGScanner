    ...
    def render(self, report: ReportDocument, *, verbose: bool = False) -> str:
        ...
        if report.duplicate_groups:
            ...
            for group in report.duplicate_groups:
                lines.append(
                    f"  {group.category}: similarity={group.similarity:.2f}; "
                    f"estimated redundant tokens={group.estimated_redundant_tokens}"
                )
                if not group.members:
                    lines.append("    Comparison details unavailable; rerun the scan.")
                for member in group.members:
                    role = "reference" if member.canonical else "match"
                    location = member.source or "unknown"
                    if member.page is not None:
                        location += f" · page {member.page}"
                    if member.line_start is not None:
                        location += f" · line {member.line_start}"
                        if member.line_end not in (None, member.line_start):
                            location += f"–{member.line_end}"
                    lines.append(f"    {role}: {location}")
                    if member.evidence_excerpt:
                        lines.append(f"      Compared excerpt: {member.evidence_excerpt}")
...
class HtmlReporter:
    ...
    def render(self, report: ReportDocument, *, limits: ReportLimits | None = None) -> str:
        ...
        for item in report.duplicate_groups:
            locations = "<br>".join(
                f"<strong>{'Reference' if member.canonical else 'Match'}:</strong> "
                f"{esc(member.source or 'unknown')}"
                + (f" · page {member.page}" if member.page is not None else "")
                + (f" · line {member.line_start}" if member.line_start is not None else "")
                + (
                    f"–{member.line_end}"
                    if member.line_end not in (None, member.line_start)
                    else ""
                ...
                + (
                    f"<blockquote>{esc(member.evidence_excerpt)}</blockquote>"
                    if member.evidence_excerpt
                    else ""
                )
            ...
            duplicate_rows.append(
                ...
                f"<td>{locations or 'Comparison details unavailable; rerun the scan.'}</td>"
                f"<td>{item.similarity:.2f}</td>"
                f"<td>{item.estimated_redundant_tokens} (estimate)</td></tr>"
            )
        duplicate_rows_html = (
            "".join(duplicate_rows) or '<tr><td colspan="4">No duplicate groups reported.</td></tr>'
        )
...
<section aria-labelledby="duplicates"><h2 id="duplicates">Duplicate analysis</h2><p>Compare every location before acting. The reference is not an automatic keep/delete decision. Redundant token and character values are estimates.</p><table><thead><tr><th>Type</th><th>Compared locations and excerpts</th><th>Similarity</th><th>Redundant tokens</th></tr></thead><tbody>{duplicate_rows_html}</tbody></table></section>
