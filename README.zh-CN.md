...
输出必须通过架构验证。RAGScanner 可从常见本地模型包装（JSON 代码块、推理前缀或序列化 JSON
字符串）中接受一个无歧义的分析对象；无效输出会使用不含证据片段、最多 6,500 个字符的紧凑上下文
和单字段 JSON 架构重试一次。Ollama 为分析预留 16,384 token 的上下文窗口。
